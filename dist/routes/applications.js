...
const applicationStateMachine_1 = require("../services/applicationStateMachine");
const applyCampaignSchema = zod_1.z.object({
...
async function applicationRoutes(fastify) {
    ...
    const stateMachine = new applicationStateMachine_1.ApplicationStateMachine(fastify.prisma);
    // POST /applications
    ...
    fastify.post('/applications/:id/transition', async (request, reply) => {
        ...
        try {
            ...
            const isCompanyOwner = user.role === 'COMPANY' && app.campaign.companyId === user.sub;
            if (!isInfluencerOwner && !isCompanyOwner && user.role !== 'ADMIN') {
                return reply.status(403).send({ error: 'Forbidden' });
            }
            const updated = await stateMachine.transition(id, status, user.sub, reason);
        ...
        try {
            if (user.role === 'INFLUENCER') {
                ...
                const companyApps = await fastify.prisma.campaignApplication.findMany({
                    where: { campaign: { companyId: user.sub } },
                    include: {
